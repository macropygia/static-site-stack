  ...
  return {
    ...
    load(id: string) {
      if (path.extname(id) === '.html') {
        if (pathMap.has(id)) {
          const compiledTemplate = compileFile(pathMap.get(id)!, options) // eslint-disable-line @typescript-eslint/no-non-null-assertion
          const html = compiledTemplate(locals)
          logger.info(
            ansis.cyanBright('[vite-plugin-pug-static] ') +
              ansis.green('compiled: ') +
              ansis.yellow(path.relative(__dirname, pathMap.get(id)!)) // eslint-disable-line @typescript-eslint/no-non-null-assertion
          )
          return html
        }
